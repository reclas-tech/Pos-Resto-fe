...
const DataTableKitchen: React.FC<KitchenInterface> = ({
  ...
  // edit
  const [selectedItem, setSelectedItem] = useState<KitchenEdit | null>(null);
  const id = selectedItem?.id ?? "";
  const [loading, setLoading] = useState(false);
  // GET ONE
  const { data: dataUser } = useGetKitchenOne(id as string);
  ...
  // GET ONE
  //
  const { handlePostSubmit } = putSubmitKitchen(id);
...
export default DataTableKitchen;
