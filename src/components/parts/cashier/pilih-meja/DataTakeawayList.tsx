...
const DataTakeawayLIst: React.FC<DataTakeawayListProps> = ({ data, onDetailModal, onPaymentModal }) => {
    ...
    const filteredData = activeFilterTakeAway === "Semua"
        ? data
        : data?.filter(item =>
        ...
        <>
            ...
            <div className="flex gap-2 pb-4">
                {["Semua", "Belum Bayar", "Sudah Bayar"].map((filter) => (
                    <button
                        key={filter}
                        className={`rounded-3xl text-xs p-1 px-2 h-fit border ${activeFilterTakeAway === filter
            ...
            <div className="overflow-auto h-screen space-y-2 scroll-container">
                {filteredData?.map((takeawayList, index) => {
                    ...
                    return (
                        <div key={takeawayList?.id || index} className="border rounded-xl text-xs">
                            <div className="flex justify-between border-b p-2">
                                <div className="space-y-0">
                                    <div className="flex gap-4">
                                        ...
                                        <div className={`w-fit h-fit rounded-3xl text-[10px] p-1 pl-1.5 pr-1.5 capitalize ${takeawayList?.status === "sudah bayar"
